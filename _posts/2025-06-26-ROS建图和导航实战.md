...
本文使用 ROS1 noetic 做的相关尝试，使用的 SLAM 算法主要是 gmapping 和 cartographer，导航算法即 navigation 框架。
...
**关于 cartographer 是否需要修改源码的问题**：个人认为不需要，现在需要修改源码的需求主要体现两个方面：全局定位初始化和 RVIZ 中手动定位（订阅 /initialpose 主题）。对于前者，目前主流的修改方案效果并没有比其内置的自动重定位机制好，对于后者，这个功能可以通过调用服务`start_trajectory`服务可以传入初始位姿，所以可自行实现一个节点处理，例如[Python script to set initial pose for pure-localization via rviz · Issue #1652 · cartographer-project/cartographer_ros](https://github.com/cartographer-project/cartographer_ros/issues/1652)（这个需求有很多人提过）。不修改源码的话，维护起来会更简单，因为这样不会引入新的问题，也不需要对其版本跟踪，大大降低了不必要的关注度。相关链接如下（一些检索思路：在 [cartographer-project/cartographer_ros](https://github.com/cartographer-project/cartographer_ros) 和 [cartographer-project/cartographer](https://github.com/cartographer-project/cartographer) 的 issues 和 PR 中搜索 `rviz`、`initialpose`、`PerformGlobalLocalization`等关键字，可以快速找到相关话题，且讨论得比较深入）：

- [给 Cartographer 添加全局重定位功能 - 哔哩哔哩](https://www.bilibili.com/opus/826735347140395032)
...
- [Python script to set initial pose for pure-localization via rviz · Issue #1652 · cartographer-project/cartographer_ros](https://github.com/cartographer-project/cartographer_ros/issues/1652)
