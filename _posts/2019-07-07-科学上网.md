...
last_modified_time: 2020-06-22 16:07:46 +0800
---
...
> 
> ——引用自[突破网络审查 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E7%AA%81%E7%A0%B4%E7%BD%91%E7%BB%9C%E5%AE%A1%E6%9F%A5)
...
> 
> ——引用自[防火长城 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E9%98%B2%E7%81%AB%E9%95%BF%E5%9F%8E)
...
* 其他：
  ...
  来自 <https://backu9.blogspot.com/2018/09/ssssr_18.html>
...
* [通用代理服务器](#通用代理)：起初 GFW 没有现在这么强大的时候，大家可以通过简单的通用代理服务器上网。这里的通用代理服务器指的是 HTTP 代理服务器和 SOCKS 代理服务器。其中 HTTP 代理服务器最为常用。
...
![基于GET-POST方法的HTTP代理原理.png](/科学上网/基于GET-POST方法的HTTP代理原理.png)
...
>
> ——引用自[请问在设置http/https代理后DNS的解析还是通过proxy吗?? · Issue #963 · FelisCatus/SwitchyOmega](https://github.com/FelisCatus/SwitchyOmega/issues/963#issuecomment-270055221)
...
![系统设置中的手动代理设置为127.0.0.1时IE中代理设置的变化.png](/科学上网/系统设置中的手动代理设置为127.0.0.1时IE中代理设置的变化.png)
...
> 
> ——引用自 [为什么以 Windows 应用商店为代表的 UWP 不能使用代理？ - 知乎](https://www.zhihu.com/question/62599100)中 Afanyiyu 的回答
...
> 
> ——引用自 [代理自动配置 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E4%BB%A3%E7%90%86%E8%87%AA%E5%8A%A8%E9%85%8D%E7%BD%AE#PAC%E6%96%87%E4%BB%B6)
...
> 
> ——引用自 [Windows 10 does not read a PAC file referenced by a file protocol](https://support.microsoft.com/en-us/help/4025058/windows-10-does-not-read-a-pac-file-referenced-by-a-file-protocol)
...
![SOCKS协议控制流程图.png](/科学上网/SOCKS协议控制流程图.png)
...
#### 在 VPS 上搭建 SOCKS 代理
使用 [SS5](https://ss5.sourceforge.net/) 即可
...
>
> ——引用自 [SOCKS # socks4a - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/SOCKS#SOCKS4a)
...
> 
> ——引用自 [SOCKS # SOCKS5 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/SOCKS#SOCKS5)
...
> 
> ——引用自 [SSH tunneling error: "channel 1: open failed: administratively prohibited: open failed" - Unix & Linux Stack Exchange](https://unix.stackexchange.com/questions/14160/ssh-tunneling-error-channel-1-open-failed-administratively-prohibited-open)
...
> 
> ——引用自[Shadowsocks - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/Shadowsocks)
...
> 
> ——引用自[Shadowsocks#ShadowsocksR - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/Shadowsocks#ShadowsocksR)
...
**温馨提示**：对于 shadowsocksr-csharp（Windows SSR 客户端）而言，它的本地代理是个全能代理，同一端口支持socks4/socks4a/socks5/http（通过 privoxy）（参见 [BRITE'S BLOG.人生在世，看得穿，又看得远者prevail everywhere.: ShadowsocksR CSharp](https://www.briten.info/2015/09/shadowsocksr-csharp.html)）
...
> 
> ——引用自[Project V · Project V 官方网站](https://v2ray.com/)
...
> 
> ![V2Ray工作原理.png](/科学上网/V2Ray工作原理.png)
> 
...
官方实现的是核心模块 V2Ray core，没有图形界面，但基本够用；其它实现使用官方的 V2Ray core 作为后端，制作了很多图形界面，具体参见 [神一样的工具们 · Project V 官方网站](https://www.v2ray.com/awesome/tools.html)
...
> 
> ——引用自[内容分发网络 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E5%85%A7%E5%AE%B9%E5%82%B3%E9%81%9E%E7%B6%B2%E8%B7%AF)
...
那么我们为什么不伪装成 HTTP 或者 HTTP/2 呢？对于 HTTP，我能想到的一个可能的原因是使用 HTTP 的 CDN 只支持静态内容，但是我们要用的是动态的，在 CDN 上根本就不会有缓存（待例证及进一步分析）；而对于 HTTP/2，则是因为 cloudflare 不完全支持 HTTP/2 [^v2ray_h2_cf]
...
> 
> ——引用自[WebSocket - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/WebSocket)

关于 WebSocket 的更多内容，还可以参考 [WebSocket 教程 - 阮一峰的网络日志](https://www.ruanyifeng.com/blog/2017/05/websocket.html)

### 具体步骤
参见 [拯救被墙的IP，CDN + v2ray，安全的科学上网方法 ｜ sprov](https://blog.sprov.xyz/2019/03/11/cdn-v2ray-safe-proxy/comment-page-7/)（以 cloudflare 为例）。

我使用的便是该教程。只不过其中的二级域名我用的是[花生壳（oray）](https://domain.oray.com/check.php)的（因为之前我用过它的壳域名）而不是 godaddy 的。花生壳注册域名需要实名认证，而 godaddy 不需要，所以事实上更推荐 godaddy 或者其它国外的域名注册站点，我当初只是年少无知，以为注册域名都需要实名认证 :joy:

下面根据该教程总结了一下大概步骤：
1. 购买一个二级域名。例如`wsxq2.top`。下面列几个购买站点：
   * 国外：
     1. [godaddy](https://www.godaddy.com/)（全球最大）
     1. namesilo（价格最低）
     1. namecheap（口碑最好）
   1. [oray](https://domain.oray.com/domain)
   1. dnspod

1. 选择一个 CDN 服务提供商并添加二级域名站点。例如在添加站点处输入`wsxq2.top`。下面列几个 CDN 服务提供商站点：
   1. [cloudflare](https://dash.cloudflare.com)（CF）
   1. [腾讯](https://cloud.tencent.com/product/cdn/pricing)
   1. [又拍云](https://www.upyun.com/pricing)（专业，较贵）
   1. [阿里](https://cn.aliyun.com/price/product#/cdn/detail)（较便宜），具体步骤可参见[V2Ray套阿里云CDN：差点翻车，常规方法走不通](https://www.youtube.com/watch?v=L5AEN5v_VmI)
   1. [NodeCache](https://www.nodecache.com/Pricing)
   1. [知道创宇](https://defense.yunaq.com/jsl/#price)
...
> 
> ——引用自[传输层安全性协议 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E5%82%B3%E8%BC%B8%E5%B1%A4%E5%AE%89%E5%85%A8%E6%80%A7%E5%8D%94%E5%AE%9A)

### 具体步骤
这里所谓的具体步骤是在前述的 [Vmess + WS + CDN](#vmess--ws--cdn) 的基础上进行的，即假设你已经有了一个二级域名，并使用了一个 CDN。
...
上一个方案（Vmess + WS + TLS + CDN）存在一个问题，就是当你用浏览器直接访问你的 VPS 的 443 端口时，会显示`400 Bad Request`，在 GFW 看来，这多少有些可疑（一个无法访问的页面，却有着巨大的流量），并且，如果它推算到这可能是一个 Vmess + WS + TLS + CDN 的科学上网方案，那么它会各种试探，这些试探一方面给你的 VPS 性能造成影响，一方面增加被发现的风险（给 GFW 提供分析素材）。因此，将它伪装成一个正常的 Web 网站或许更加安全，这应该才是网上广为流传的“最安全的科学上网方式”
...
> 
> 反向代理在现时的互联网中并不少见，而另一些例子，像是CDN、SNI代理等，是反向代理结合 DNS 的一类延伸应用。
> 
> ——引用自[反向代理 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86)

反向代理在本方案中的应用如下：

在 VPS 的 443 端口搭建一个 HTTPS Web 服务器，然后构建一个“像模像样”的网站以混淆视听。当访问特定路径时（比如`/awesomepath`），将请求转发给 v2ray 处理，v2ray 作为一个在特定端口（如`127.0.0.1:12345`）监听的`Vmess + WS`服务，接收到请求后再向实际服务器（比如`https://google.com`）发起请求，得到响应后将响应转发回 VPS 上的 Web 服务器，VPS 上的 Web 服务器再将响应转发回 v2ray 客户端。外界只能感知到 Web 服务的存在，而感知不到在 Web 后面的 v2ray 的存在。

### 具体步骤
同样地，这里所谓的具体步骤是在前述的 [V2Ray + WS + TLS + CDN](#vmess--ws--tls--cdn) 的基础上进行的，即假设你已经有了一个二级域名，并使用了一个 CDN，且配置了 TLS 证书。

具体步骤如下（以 CentOS 7 为例）：
1. 关掉 CDN 以便于测试。
   
   在 [Cloudflare 控制面板](https://dash.cloudflare.com/login) 的 DNS 处将你的 VPS 的域名设置如下：
   ```
...
1. 修改 V2ray 服务配置。
   ...
   将监听端口改为你喜欢的端口（如 12345）；删除 TLS 相关的配置（这里只需要用到`Vmess + WS`，TLS 交给前面的 Web 服务）。即改成类似于[这样的配置文件](#server)
...
1. 安装并启用你喜欢的 Web 服务，测试。
   ...
   测试方法很简单，访问你的 Web 服务器（假设你的 VPS 使用的域名是`www.example.com`），看是否出现默认页面：<https://www.example.com>
...
1. 为提供 Web 服务的软件配置证书，测试。

   修改提供 Web 服务的软件（这里是 lighttpd ）的配置文件中的**SSL 配置**部分即可：
   ```
   #SSL配置，依赖 mod_ssl
   ...
   完整配置可参考后面的 [参考配置文件-Web-lighttpd](#lighttpd)

   测试：<https://www.example.com>

1. 配置反向代理到 V2ray，测试。
   
   修改提供 Web 服务的软件（这里是 lighttpd ）的配置文件中的**反向代理**部分即可：
   ```
   #WS反向代理，依赖 mod_proxy
...
1. 修改客户端配置文件，测试。
   ...
   修改 V2ray 客户端配置文件如下：参见后面的 [参考配置文件-V2ray-client](#client)
...
另请参见下列文章：
* [V2Ray高级技巧：流量伪装 - tlanyan](https://tlanyan.me/v2ray-traffic-mask/)
* [v2ray带伪装一键脚本 - 网络跳越](https://www.hijk.pw/v2ray-one-click-script-with-mask/)
...
## 通过已经可以科学上网的电脑实现科学上网
也就是说，如果你有一台设备通过上述的方法（SSH/SS/SSR/V2Ray/V2Ray+WS+CDN/V2Ray+WS+TLS+CDN）之一实现了科学上网，那么你就可以借助那台设备轻松地让其它和那台设备**属于同一局域网的设备**实现科学上网。比如你的实体机（如 Windows）实现了科学上网，那么对于你的 kali 虚拟机你就没必要想尽各种办法让它与你的实体机进行类似的配置以实现科学上网，你只需让虚拟机和 Windows 主机处于同一局域网下即可（对于 Virtual Box 可使用**仅主机网络**）。
...
更多 Linux 下的网络工具请参见： [Linux常用网络工具总结 - int32bit的博客 ｜ int32bit Blog](https://int32bit.me/2016/05/04/Linux%E5%B8%B8%E7%94%A8%E7%BD%91%E7%BB%9C%E5%B7%A5%E5%85%B7%E6%80%BB%E7%BB%93/)

#### 在线工具
* 域名到期查询、whois、ip、同IP网站、DNS、网站备案、ping、网站测速、traceroute、CDN观测: <https://ping.chinaz.com/>
...
* 网站测速、ping、traceroute、nslookup、网速测试：<https://www.webkaka.com/shanxi.aspx>
...
1. 确保你的服务器可以访问谷歌，使用如下命令：
   ...
   简要解释下：curl是一个强大的用于在服务器和客户端间传输数据的工具。上述命令使用的是其最常用的一个能力，获取 Web 网页。其中：
     ...
     * `www.google.com`参数用于指明从<https://www.google.com>获取数据；
...
| 2019-11-29 | 3.0  | 一个下午              | 完善了 [V2Ray + WS + CDN](#vmess--ws--cdn) 部分和 [V2Ray + WS + TLS + CDN](#vmess--ws--tls--cdn) 部分                                                                                                                                                       |
| 2019-12-05 | 4.0  | 一个上午（2小时）     | 添加了[一些尝试](#一些尝试)部分                                                                                                                                                                                                                             |
| 2020-06-19 | 5.0  | 一个下午（3小时）     | 添加了一些推荐的“机场”、 VPS 提供商、域名购买站点，添加了“真·最安全的科学上网方式”（[Vmess + WS + TLS + CDN + Web](#vmess--ws--tls--cdn--web)），修正了一些链接                                                                                             |
...
* [BRITE'S BLOG.人生在世，看得穿，又看得远者prevail everywhere.: ShadowsocksR CSharp](https://www.briten.info/2015/09/shadowsocksr-csharp.html)
...
* [Linux常用网络工具总结 - int32bit的博客 ｜ int32bit Blog](https://int32bit.me/2016/05/04/Linux%E5%B8%B8%E7%94%A8%E7%BD%91%E7%BB%9C%E5%B7%A5%E5%85%B7%E6%80%BB%E7%BB%93/)
