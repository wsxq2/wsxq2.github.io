...
表示层：    负责翻译、加密和压缩数据  
会话层：    负责对话控制和同步  
传输层：    负责**报文**从一个**进程**到另一个**进程**的传递  
网络层：    负责**分组**从一个**逻辑地址**到另一个**逻辑地址**的传递  
数据链路层：负责**帧**从一**跳（节点）**到下一**跳（节点）**的传递  
...
应用层：DNS NFS SNMP TFTP FTP POP SMTP Telnet  
传输层：UDP TCP  
网络层：IP ICMP IGMP  
数据链路层：ARP RARP  
...
1. 域名系统：
   ...
   3. Internet的DNS结构：![DNS层次结构](http://wsxq12.55555.io/DNS层次结构.png)
