...
一些阅读心得：
- 关于行为树：行为树是对 FSM（有限状态机） 的替代，它的优点是更加模块化，更容易实现复杂的响应式任务。行为树在 [1709.00084](https://arxiv.org/pdf/1709.00084) 中有详细说明（阅读第 1 章节就足够了），一个比较知名的 C++ 库是 [BehaviorTree.CPP](https://www.behaviortree.dev/)，nav2 中也使用的该库，且对其进行了一定程度的封装，以便在 nav2 中使用。nav2 中提供了大量的行为树节点，其中 Action 插件是最多的，以便于用户使用。在 nav2中，行为树使用 xml 文件表达。
