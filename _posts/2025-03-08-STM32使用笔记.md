...
相比 nvim，VSCode 的使用体验更好，尤其是对于新手来说，VSCode 的使用门槛更低。下面简单介绍下 VSCode 的使用方法。

1. 使用 STM32CubeMX 生成代码时选择 cmake 方式
1. 在 vscode 中安装以下插件：

   - clangd: 提供代码补全、语法检查等功能。安装后打开工程时会提示你下载 clangd，确认下载即可。这个插件会和 cpptools（C/C++插件） 冲突，需要将 cpptools 卸载掉。cpptools 是微软官方的插件，功能类似于 clangd，但不如 clangd 好用。
   - cortex-debug：提供调试功能
   - github copilot：提供 AI 代码补全功能，提高代码编写效率
   - github copilot chat：提供 AI 聊天功能，辅助代码编写
   - STM32 VS Code Extension（又名STM32Cube for Visual Studio Code）: ST 官方插件，结合 STM32CubeMX 提供代码生成、文档查看等功能。它依赖于[STM32CubeCLT](https://www.st.com/en/development-tools/stm32cubeclt.html)，这个需要手动下载安装。

1. 在 vscode 中打开 STM32 项目文件夹，vscode 会提示 CMakeLists.txt 选择，此处选择 STM32CubeMX 生成的 CMakeLists.txt 文件（如果没提示，就是它自己找到了，不用管），然后它会提示配置预设项（CMake presets），选择 Debug 即可。然后项目会自动配置和初始化

遇到的问题根据 vscode 的提示进行操作即可。
...
如果仅下载程序到板子上，可以使用以下命令：

```ps1
cd ./hardware/ys/build/Debug
STM32_Programmer_CLI.exe -c port=SWD -d master_ys.elf -rst
```
