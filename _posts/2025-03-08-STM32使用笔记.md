...
1. 在 vscode 中安装以下插件：
   ...
   - clangd: 提供代码补全、语法检查等功能。安装时会提示你下载 clangd，确认下载即可。
   - cortex-debug：提供调试功能
   - github copilot：提供 AI 代码补全功能，提高代码编写效率
   - github copilot chat：提供 AI 聊天功能，辅助代码编写
   - STM32 VS Code Extension: ST 官方插件，结合 STM32CubeMX 提供代码生成、文档查看等功能。它依赖于[STM32CubeCLT](https://www.st.com/en/development-tools/stm32cubeclt.html)，需要手动下载安装。

1. 在 vscode 中打开 STM32 项目文件夹，vscode 会提示 CMakeLists.txt 选择，此处选择 STM32CubeMX 生成的 CMakeLists.txt 文件，然后它会提示选择 Kits，此处选择 arm-none-eabi kits。然后项目会自动配置和初始化
