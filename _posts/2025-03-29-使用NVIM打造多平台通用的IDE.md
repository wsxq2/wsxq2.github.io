...
首先我们需要安装 nvim，安装方法请参考[官方文档](https://neovim.io/)。安装完成后，我们可以开始配置 nvim。
...
该组件安装后，在 mason 插件方面, 安装了 clangd。clangd 作为 LSP，用于代码补全、编译错误检查、定义跳转、代码格式化。但需要注意的是, `clangd`本身不支持代码格式化，但它可以和`clang-format`集成实现该功能（实测发现没有手动安装`clang-format`也能通过`<Space>cf`进行格式化，难道 LazyVim 会自动集成 clang-format 吗？这块还没搞明白）。
...
[LazyVim 官方文档]: https://www.lazyvim.org/installation
