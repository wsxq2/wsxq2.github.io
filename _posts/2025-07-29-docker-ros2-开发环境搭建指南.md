...
本文介绍了如何使用 Docker 容器化技术搭建 ROS2 开发环境，包括官方镜像使用、最佳实践、常用配置模板以及问题解决方案。通过容器化部署，可以实现环境隔离、快速部署和跨平台兼容，大大提升 ROS2 开发效率。

本文坚持“不造重复轮子”的原则，如果某个主题有相关教程（如 Ubuntu 中安装 Docker），则不会详细的描述具体的步骤，而是指出相应链接。如有不熟悉的基础概念，建议自行搜索了解，包括 ROS2、Docker 容器、Docker 镜像、Dockerfile、docker compose等。
...
从 [Google Drive](https://drive.google.com/file/d/1v9qJ2U7GlYhqBJr7OQHWbDSCfgiVaLWb/view?usp=drive_link) 下载 bag 文件并解压，解压后要保证目录结构如下所示：
...
> 这部分说明了三种情形：ROS1 Noetic、ROS1 Noetic + Docker、ROS2 Humble。只有第二种情形会用到其他文件，如 docker 相关文件等，第二种不仅仅实现了 FAST-LIVO2 的编译和运行，还实现了开发环境的搭建，由于使用 Docker，所以是一个“容易移植”的开发环境。另外两种相当于是教程，作为参考。
...
```json
{
    ...
    "customizations": {
        "vscode": {
            ...
            "settings": {
                "http.proxy": "${localEnv:https_proxy}",
...
##### 使用方法
从 [fast-livo2-dataset - OneDrive](https://connecthkuhk-my.sharepoint.com/personal/zhengcr_connect_hku_hk/_layouts/15/onedrive.aspx?id=%2Fpersonal%2Fzhengcr%5Fconnect%5Fhku%5Fhk%2FDocuments%2Ffast%2Dlivo2%2Ddataset&ga=1) 下载即可。建议下载最小的 Retail_Street.bag。解压并放置到 data/ 目录。
...
然后在 WSL 中使用`code .`命令运行 VS Code，然后点击左下角，选择“Reopen folder in container”，等待一段时间后即可自动完成环境搭建，然后打开 VS Code 中的 Terminal，先进行编译操作和 source：
...
转换完成后需要修改消息类型，在 metadata.yaml 中做出以下修改：
