...
3. **配置 Docker Desktop**
   - **网络代理**：Settings → Resources → Proxies（假设本地代理为 `http://127.0.0.1:7890`）
   - **WSL 集成**：Settings → Resources → WSL integration
     - 勾选 "Enable integration with my default WSL distro"
     - 重启后手动勾选相应发行版启用

4. **下载测试数据**
   从 [Google Drive](https://drive.google.com/file/d/1v9qJ2U7GlYhqBJr7OQHWbDSCfgiVaLWb/view?usp=drive_link) 下载 bag 文件并解压：
