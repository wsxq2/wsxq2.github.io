...
本文坚持“不造重复轮子”的原则，如果某个主题有相关教程（如 Ubuntu 中安装 Docker），则不会详细的描述具体的步骤，而是指出相应链接。如有不熟悉的基础概念，建议自行搜索了解。

本文重点在于 Docker 和 ROS2 结合使用这一场景，对于 Docker 本身的使用指南建议查阅 [Docker 使用指南]({% post_url 2022-03-16-Docker使用笔记 %})
...
ROS1 humble 不是 FAST-LIVO2 官方支持的版本，使用的是 [网友移植的版本](https://github.com/integralrobotics/FAST-LIVO2)。目录结构和 [前面](#ros1-noetic--docker) 基本相同。
