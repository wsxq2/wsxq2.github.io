...
1. KoolShare 论坛 LEDE 固件: [LEDE - KoolShare - 源于玩家 服务玩家](http://koolshare.cn/forum-97-1.html)。本文用的便是这个
...
### 软路由配置
在笔记本的浏览器中输入`192.168.1.1`访问软路由 Web 配置界面，用户为`root`，密码为`koolshare`（以上内容在[下载页面](http://firmware.koolshare.cn/LEDE_X64_fw867/)处有说明）
