...
4. 触动精灵相关知识：
   1. [触动精灵脚本开发手册](http://www.touchsprite.com/helpdoc#/doc?id=1625)
   2. [TSLib触动精灵基础拓展库使用手册](http://www.touchsprite.com/helpdoc#/doc?id=16405)
   3. [触动精灵IDE脚本编辑器使用手册](http://www.touchsprite.com/helpdoc#/doc?id=1663)
   4. [触动精灵抓色器使用手册](http://www.touchsprite.com/helpdoc#/doc?id=1657)
   5. [触动精灵 Android 模拟器使用手册](http://www.touchsprite.com/helpdoc#/doc?id=1119)
   6. [触动精灵脚本开发实例代码](http://www.touchsprite.com/helpdoc#/doc?id=1742)
   7. [手机脚本开发实用技巧](http://www.touchsprite.com/helpdoc#/doc?id=1812)
   8. [常见问题](http://www.touchsprite.com/helpdoc#/doc?id=1365)
...
* 使用`adb devices`查看正在运行的设备
* `adb -s {127.0.0.1:<指定端口>|<模拟器标识>}` 可连接任意指定的设备
* 连接方法：

  ```
  cd <adb.exe默认路径> #如果已添加了前述的环境变量可省略这一步
  adb kill-server #很多时候连不上，就是因为没有kill
  adb devices #查看正在运行的设备
  ```

1. 夜神：
   * `adb.exe`默认路径为：`D:\Program Files (x86)\Nox\bin\adb.exe`
   * 端口规律：第一个模拟器端口是62001，第二个模拟器端口是62025，第三个是62025+1，以此类推
...
* `shell`: 执行在安卓上执行的shell命令
  ...
  * `am`: Activity Manager。该命令用来执行一些系统动作, 例如启动指定activity, 结束进程, 发送广播, 更改屏幕属性等. 调试利器.
    * `start`: 根据intent指向启动Activity, Intent可以是显示的指向activity, 也可以是ACTION方式, 并且可以添加flag, data等参数信息.
      ```
      adb shell am start com.tencent.android.qqdownloader/com.tencent.assistant.activity.SplashActivity
      ```
    * `force-stop`: 关闭应用
      ```
