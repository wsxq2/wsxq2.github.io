---
tags: [Vim]
last_modified_time: 2019-04-20 21:38:33 +0800
---
...
:1,/^<!-- vim-markdown-toc GFM -->/ s/[^!]\(\[[^]]\+\]\[[^]]\+\]\)/\=setreg('T','* '.submatch(1),'l')/egn
:/^<!-- vim-markdown-toc -->/,/^<!-- link start -->/ s/[^!]\(\[[^]]\+\]\[[^]]\+\]\)/\=setreg('T','* '.submatch(1),'l')/egn
/<!-- link start -->
"tp
...
:/<!-- abbreviations start -->/+1,/<!-- abbreviations end -->/-1 ! sed -nr -e 's/^.*\b([A-Z]{2,})\b.*$/*[\1]: /p' 2018-12-02-16位汇编程序设计.md | sort | uniq
   ...
   2.
     ```
     qa
:/^<!-- abbreviations start -->/+1,/^<!-- abbreviations end -->/-1 g!/^$/d
:let @t=''
:/^<!-- vim-markdown-toc -->/,/^<!-- link start -->/ s/\C[^%]\([A-Z]\{2,}\)\>/\=setreg('T',submatch(1),'l')/egn
/^<!-- abbreviations start -->
"tp
:/^<!-- abbreviations start -->/+1,/^<!-- abbreviations end -->/-1 sort u

:/^<!-- abbreviations start -->/+1,/^<!-- abbreviations end -->/-1 !$bl/handle-abbreviations.sh
     ```
