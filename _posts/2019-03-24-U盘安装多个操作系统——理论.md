...
本文原为一篇文章，分为预备知识和实际操作，后来发现预备知识过于庞杂，故而分成两篇文章，一为理论，二为实践，本文即为理论。实践部分参见 [U盘安装多个操作系统——实践](https://wsxq2.55555.io/blog/2019/03/24/U盘安装多个操作系统——实践)
...
* [UEFI](#uefi)
  * [简介](#简介-2)
  * [统一可扩展固件接口（UEFI）的产生](#统一可扩展固件接口uefi的产生)
  * [比较统一可扩展固件接口（UEFI）和BIOS](#比较统一可扩展固件接口uefi和bios)
  * [采用UEFI固件的x86/x64系统类别](#采用uefi固件的x86x64系统类别)
  * [UEFI Shell](#uefi-shell)
    * [获取 UEFI Shell](#获取-uefi-shell)
    * [启动 UEFI Shell](#启动-uefi-shell)
    * [重要 UEFI Shell 命令](#重要-uefi-shell-命令)
      * [help](#help)
      * [bcfg](#bcfg)
      * [map](#map)
      * [edit](#edit)
...
#### 启动 UEFI Shell
3个方法：
1. 主板支持。部分基于 AMI Aptio x86_64 UEFI 固件的主板 (从 Sandy Bridge 起，尤其是华硕) 提供了一个叫做 "Launch EFI Shell from filesystem device" 的选项。对于这些主板，下载 x86_64 UEFI Shell ，复制进 EFI 系统分区，命名为`<EFI_SYSTEM_PARTITION>/shellx64.efi`(大部分是`/boot/efi/shellx64.efi`).
...
更多信息参见 [Intel 技术文章-EFI Shells and Scripting](http://software.intel.com/en-us/articles/efi-shells-and-scripting/) 和 [UEFI_Shell_Spec_2_0.pdf][uefi-shell-2]
...
>
> ![GUID_Partition_Table_Scheme.svg](http://wsxq12.55555.io/U盘安装多个操作系统/GUID_Partition_Table_Scheme.svg)
> 
> ——引用自 [全局唯一标识分区表 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/GUID%E7%A3%81%E7%A2%9F%E5%88%86%E5%89%B2%E8%A1%A8)
...
> </td></tr></tbody></table>
> 
> &emsp;&emsp;主分区表和备份分区表的头分别位于硬盘的第二个扇区（LBA 1）以及硬盘的最后一个扇区。备份分区表头中的信息是关于备份分区表的。
> 
> ——引用自 [全局唯一标识分区表 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/GUID%E7%A3%81%E7%A2%9F%E5%88%86%E5%89%B2%E8%A1%A8)
...
> <td>苹果RAID分区，下线
> </td>
> <td><tt>52414944-5F4F-11AA-AA11-00306543ECAC</tt>
> </td></tr>
> <tr>
> <td>苹果启动分区
> </td>
...
> 1. ^ 具体定义见[src/sys/sys/disklabel_gpt.h](http://cvsweb.netbsd.org/bsdweb.cgi/src/sys/sys/disklabel_gpt.h?only_with_tag=MAIN)。NetBSD的GUID在单独定义之前曾经使用过FreeBSD的GUID。
> 
> ——引用自 [全局唯一标识分区表 - 维基百科，自由的百科全书](https://zh.wikipedia.org/wiki/GUID%E7%A3%81%E7%A2%9F%E5%88%86%E5%89%B2%E8%A1%A8)
...
> 
> ——引用自 [The rEFInd Boot Manager: Using rEFInd](http://www.rodsbooks.com/refind/using.html#keyboard)
...
配置详情参见： [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table1)
...
配置详情参见： [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table2)
...
```
配置详情参见： [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table3)
...
##### 分页支持
GRUB支持对长输出进行分页(比如运行help的输出).不过只有normal shell支持分页,而rescue shell不支持.开启分页支持的方法如下:
...
* [Intel 技术文章-EFI Shells and Scripting](http://software.intel.com/en-us/articles/efi-shells-and-scripting/)
* [modified UEFI Shell v2 二进制文件](http://dl.dropbox.com/u/17629062/Shell2.zip)
* [bcfg](#bcfg)
...
* [src/sys/sys/disklabel_gpt.h](http://cvsweb.netbsd.org/bsdweb.cgi/src/sys/sys/disklabel_gpt.h?only_with_tag=MAIN)
...
* [获取 UEFI Shell](#获取-uefi-shell)
* [The rEFInd Boot Manager: Installing rEFInd](https://www.rodsbooks.com/refind/installing.html#addons)
* [The rEFInd Boot Manager: Using rEFInd](http://www.rodsbooks.com/refind/using.html#keyboard)
* [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table1)
* [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table2)
* [The rEFInd Boot Manager: Configuring the Boot Manager](http://www.rodsbooks.com/refind/configfile.html#table3)
